...
\emphasizebox{barrier}是一款跨平台的共享键盘和鼠标的软件, 属于开源软件, 代码地址\url{https://gitee.com/sea18c/barrier#https://repology.org/project/barrier/versions}, 可实现在不同桌面系统上通过同一鼠标拖动到屏幕边界实现鼠标和键盘在不同系统上的桌面操作, 前提是每个系统都需要接显示屏, 这个跟远程桌面有本质差别, 因此barrier只能当作是远程鼠标和键盘, 安装步骤如下:
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{009_barrier_instal_02.jpg}
\caption{客户端配置如图}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{008_barrier_instal_01.jpg}
\caption{服务器端配置如图}
\end{figure}
...
\begin{itemize}
\item 检查vim时候有\emphasizebox{clipboard}功能
\begin{commandbox}
 > vim --version | grep clipboard
\end{commandbox}

\item 如果显示没有\emphasizebox{clipboard}功能, 使用命令安装, 实际测试vim-gnome不用安装也可以
\begin{commandbox}
 > sudo apt-get install vim vim-scripts vim-gtk vim-gnome
\end{commandbox}

\item 再次检查vim时候有\emphasizebox{clipboard}功能
\begin{commandbox}
 > vim --version | grep clipboard
\end{commandbox}
...
\item 进入\emphasizebox{~/.ssh/}目录, 生成密钥, 不用输入其他, 直接回车
\begin{commandbox}
